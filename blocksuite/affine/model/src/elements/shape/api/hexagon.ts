...
export const hexagon = {
  points({ x, y, w, h }: IBound): IVec[] {
    return flatTopHexagonPoints({ x, y, w, h });
  },
  draw(ctx: CanvasRenderingContext2D, { x, y, w, h, rotate = 0 }: IBound) {
    ...
    const points = flatTopHexagonPoints({ x, y, w, h });
    ctx.beginPath();
  ...
  includesPoint(
    ...
    const points = getPointsFromBoundWithRotation(this, hexagon.points);
  ...
  containsBound(bounds: Bound, element: ShapeElementModel) {
    const points = getPointsFromBoundWithRotation(element, hexagon.points);
  ...
  getRelativePointLocation(position: IVec, element: ShapeElementModel) {
    ...
    let points = hexagon.points(bound);
    points.push(point);
